...
const GuestBookingPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:3000'}/api/bookings/guest`, {
  ...
  if (success) {
    return (
      <Box sx={{ minHeight: '100vh', bgcolor: '#000', color: '#fff' }}>
        <Box sx={{ bgcolor: '#FFB800', py: 2, px: 3 }}>
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#000', color: '#fff' }}>
      ...
      >
        <IconButton onClick={() => navigate('/')} sx={{ color: '#000' }}>
          <ArrowBack />
        </IconButton>
        <Typography variant="h5" sx={{ fontWeight: 700, color: '#000' }}>
          Ebulan Wings - Guest Booking
        </Typography>
      </Box>
